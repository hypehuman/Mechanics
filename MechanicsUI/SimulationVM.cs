...
public class SimulationVM : INotifyPropertyChanged
{
    ...
    private static readonly PropertyChangedEventArgs sIsShrinkingToFitBodiesChangedArgs = new(nameof(IsShrinkingToFitBodies));
    private bool _isShrinkingToFitBodies;
    public bool IsShrinkingToFitBodies
    {
        get => _isShrinkingToFitBodies;
        set
        {
            _isShrinkingToFitBodies = value;
            PropertyChanged?.Invoke(this, sIsShrinkingToFitBodiesChangedArgs);
            if (value)
            {
                UpdateBounds();
            }
    ...
    private static readonly PropertyChangedEventArgs sIsExpandingToFitBodiesChangedArgs = new(nameof(IsExpandingToFitBodies));
    private bool _isExpandingToFitBodies;
    public bool IsExpandingToFitBodies
    {
        get => _isExpandingToFitBodies;
        set
        {
            _isExpandingToFitBodies = value;
            PropertyChanged?.Invoke(this, sIsExpandingToFitBodiesChangedArgs);
            if (value)
            {
                UpdateBounds();
            }
    ...
    private static readonly double FitPaddingFactor = Math.Pow(2, -8);

    private void UpdateBounds()
    {
        var shrink = _isShrinkingToFitBodies;
        var expand = _isExpandingToFitBodies;
        ...
        if (!Arrangement.ComputeBoundingBox(Model.Bodies, FitPaddingFactor, out var bodiesMin, out var bodiesMax))
        {
            return;
        }
        ...
        if (shrink && !expand)
        {
            ...
            Vector3D oldMin = new(
                Math.Min(oldBound0.X, oldBound1.X),
                Math.Min(oldBound0.Y, oldBound1.Y),
                Math.Min(oldBound0.Z, oldBound1.Z)
            );
            Vector3D oldMax = new(
                Math.Max(oldBound0.X, oldBound1.X),
                Math.Max(oldBound0.Y, oldBound1.Y),
                Math.Max(oldBound0.Z, oldBound1.Z)
            );
            Vector3D newMin = new(
                Math.Max(oldMin.X, bodiesMin.X),
                Math.Max(oldMin.Y, bodiesMin.Y),
                Math.Max(oldMin.Z, bodiesMin.Z)
            );
            Vector3D newMax = new(
                Math.Min(oldMax.X, bodiesMax.X),
                Math.Min(oldMax.Y, bodiesMax.Y),
                Math.Min(oldMax.Z, bodiesMax.Z)
            );
        ...
        throw new InvalidOperationException("Not all cases are handled!");
    }
