...
public abstract class Arrangement : IGetConstructorParameters
{
    ...
    public static bool ComputeBoundingBox(IEnumerable<Body> bodies, double paddingFactor, out Vector3D min, out Vector3D max)
    {
        var anyBodies = false;
        ...
        var padding = paddingFactor * diagonal;
        var paddingVect = new Vector3D(padding, padding, padding);
        min -= paddingVect;
        max += paddingVect;

        return anyBodies && min.IsFinite() && max.IsFinite();
    }
}
